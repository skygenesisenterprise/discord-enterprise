...
const solution = getSolution("government");
...
export async function generateMetadata({ params }: SolutionPageParams): Promise<Metadata> {
  ...
  return {
    title: tMeta("detailTitle", { solution: t("government.title") }),
    description: t("government.description"),
  };
}

export default async function GovernmentSolutionsPage({ params }: SolutionPageParams) {
  const { locale } = await params;
