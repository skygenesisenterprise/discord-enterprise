...
const solution = getSolution("b2c");
...
export async function generateMetadata({ params }: SolutionPageParams): Promise<Metadata> {
  ...
  return {
    title: tMeta("detailTitle", { solution: t("b2c.title") }),
    description: t("b2c.description"),
  };
}

export default async function B2CSolutionsPage({ params }: SolutionPageParams) {
  const { locale } = await params;
