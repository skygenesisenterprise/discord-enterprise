...
const solution = getSolution("workplace");
...
export async function generateMetadata({ params }: SolutionPageParams): Promise<Metadata> {
  ...
  return {
    title: tMeta("detailTitle", { solution: t("workplace.title") }),
    description: t("workplace.description"),
  };
}

export default async function WorkplaceSolutionsPage({ params }: SolutionPageParams) {
  const { locale } = await params;
