...
const config = getPlatformConfig("media");

export async function generateMetadata({ params }: PlatformRouteParams): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "Public.platform.pages.media.metadata" });
...
export default async function PublicMediaPlatformPage({ params }: PlatformRouteParams) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "Public.platform.pages" });
  const copy = t.raw("media") as PlatformPageCopy;
